...
//
// another_secured_service HTTP server
//
// Command:
...
// Server lists the another_secured_service service endpoint HTTP handlers.
type Server struct {
	Mounts           []*MountPoint
	Signin           http.Handler
	Secure           http.Handler
...
// New instantiates HTTP handlers for all the another_secured_service service
// endpoints.
func New(
	e *anothersecuredservice.Endpoints,
	mux goahttp.Muxer,
	...
	return &Server{
		Mounts: []*MountPoint{
			{"Signin", "POST", "/another/signin"},
			{"Secure", "GET", "/another/secure"},
			{"DoublySecure", "PUT", "/another/secure"},
			{"AlsoDoublySecure", "POST", "/another/secure"},
		},
...
func (s *Server) Service() string { return "another_secured_service" }
...
// Mount configures the mux to serve the another_secured_service endpoints.
...
// MountSigninHandler configures the mux to serve the "another_secured_service"
...
func MountSigninHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/another/signin", f)
}

// NewSigninHandler creates a HTTP handler which loads the HTTP request and
// calls the "another_secured_service" service "signin" endpoint.
...
// MountSecureHandler configures the mux to serve the "another_secured_service"
...
func MountSecureHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/another/secure", f)
}

// NewSecureHandler creates a HTTP handler which loads the HTTP request and
// calls the "another_secured_service" service "secure" endpoint.
...
// MountDoublySecureHandler configures the mux to serve the
...
func MountDoublySecureHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("PUT", "/another/secure", f)
}

// NewDoublySecureHandler creates a HTTP handler which loads the HTTP request
// and calls the "another_secured_service" service "doubly_secure" endpoint.
...
func MountAlsoDoublySecureHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/another/secure", f)
}

// NewAlsoDoublySecureHandler creates a HTTP handler which loads the HTTP
// request and calls the "another_secured_service" service "also_doubly_secure"
// endpoint.
