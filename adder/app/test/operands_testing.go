...
// AddOperandsTooManyRequests runs the method Add of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func AddOperandsTooManyRequests(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.OperandsController, left int, right int) http.ResponseWriter {
	// Setup service
	var (
		logBuf bytes.Buffer
	...
	if rw.Code != 429 {
		t.Errorf("invalid response status code: got %+v, expected 429", rw.Code)
	}
